  ...
  return (
    ...
    >
      ...
      <div className="flex flex-row items-center gap-2">
       <div><Dot size={20} color="#8c8c8c" strokeWidth={10} /></div>
       <div className="flex flex-row ">
         <div className="text-1xl font-medium text-gray-700">Step {ticket.id}:</div>
        <div className="text-1xl font-medium text-gray-700 truncate max-w-35
">{ticket.title}</div>
       </div>
      </div>
      ...
      <div className="text-blue-800 bg-sky-100 w-17 h-9 flex items-center font-medium rounded-xl p-2">{ticket.status}</div>

      <div className="font-medium text-gray-700">{ticket.date}</div>

      <div><Ellipsis size={25} color="#0a0a0a" /></div>
    </div>
  );
};
